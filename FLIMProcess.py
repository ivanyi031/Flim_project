...
FLIM_colormap = scipy.io.loadmat(PATH)
...
def accumarray_mean(mat):
     ...
     for i in range(len(c)):
         pnt_end+=c[i]
#         #print('pnt_end:',pnt_end)
         #g=np.array (m[a][pnt_start:pnt_end+1])
#         #print('g:',g)
...
#print('OptPlot_file:',OptPlot_file)
#print(OptPlot_file+'\\'+OptExp_filename[0][iExp_start-1][0:-4])
...
for iExp in range(iExp_start,iExp_end+1):
    ...
    if flag_Rsq==1:
        ...
        plt.imshow(Rsq_ref, extent=[xaxis[0], xaxis[-1], yaxis[0],yaxis[-1]],cmap=ListedColormap(FLIM_colormap['flimcolormap_black']),vmax=1,vmin=0.95)
        plt.xlabel('pixel') 
        plt.ylabel('pixel')
        ...
        plt.hist(Rsq_ref[np.nonzero(Rsq_ref)],bins=np.arange(0.8,1,0.005))
        ...
        plt.show()
        #experiment
        Rsq_exp= np.rot90(Rsq_exp,1)
        plt.imshow(Rsq_exp, extent=[xaxis[0], xaxis[-1], yaxis[0],yaxis[-1]],cmap=ListedColormap(FLIM_colormap['flimcolormap_black']),vmax=1,vmin=0.95)
        plt.xlabel('pixel') 
        plt.ylabel('pixel')
        ...
        plt.hist(Rsq_exp[np.nonzero(Rsq_exp)],bins=np.arange(0.8,1,0.005))
        plt.xlabel('R Square Value'); 
        plt.ylabel('Counts'); 
        ...
        if flag_tauPhi == 1:
            ...
            if flag_temp==1:
                ...
                if not os.path.exists(path):
                    os.mkdir(path)
            if typeProbe==1:
                plt.imshow(tauPhi_plot*1e9,extent=[xaxis[0], xaxis[-1], yaxis[0],yaxis[-1]],cmap=plt.get_cmap('jet'),vmax=500,vmin=100)
            elif typeProbe==2:
                plt.imshow(tauPhi_plot*1e6,extent=[xaxis[0], xaxis[-1], yaxis[0],yaxis[-1]],cmap=plt.get_cmap('jet'),vmax=2.5,vmin=1)
            elif typeProbe==3:
                if math.floor(math.log10(tauN2*1e9))+1 >3:
                    plt.imshow(tauPhi_plot*1e6,extent=[xaxis[0], xaxis[-1], yaxis[0],yaxis[-1]],cmap=plt.get_cmap('jet'))
                else:
                    plt.imshow(tauPhi_plot*1e9,extent=[xaxis[0], xaxis[-1], yaxis[0],yaxis[-1]],cmap=plt.get_cmap('jet'))
        plt.xlabel('pixel')
        plt.ylabel('pixel')
        ...
        M=M*IM
        Phi=(-coef_exp[:,:,3]+coef_ref[:,:,3])+math.atan(2*math.pi*fmod*tauRef)
        Phi=Phi*IM
        ...
        plt.plot(phasor_x_ref[0],phasor_y_ref[0],'k-o',ms=10,lw=2)
        plt.plot(phasor_x_ref[1],phasor_y_ref[1],'o-k',ms=10,lw=2)
        if typeProbe==1:
            plt.plot(phasor_x_ref[2],phasor_y_ref[2],'o-k',ms=10,lw=2)
        plt.xlim([0,1.0])
        plt.ylim([0,0.6])
        ...
        if flag_O2==1:
            ...
            if flag_temp==1:
                ...
                if not os.path.exists(path):
                    os.mkdir(path)
            plt.imshow(O2exp_plot,extent=[xaxis[0], xaxis[-1], yaxis[0],yaxis[-1]],cmap=plt.get_cmap('jet'),vmax=25,vmin=0)
            plt.xlabel('pixel')
            plt.ylabel('pixel')
        ...
        if flag_O2grad == 1:         
            ...
            O2exp_up=smooth(O2exp[8,:],spansmooth)*100
            O2exp_low=smooth(O2exp[1008-8+1,:],spansmooth)*100
            O2exp_mean=accumarray_mean(np.transpose(O2exp))
            O2exp_mean=smooth(O2exp_mean,spansmooth)*100
            print(O2exp_mean)
            plt.plot(xaxis,O2exp_up,'b.',ms=5,label='upstream')
            plt.plot(xaxis,O2exp_low,'r.',ms=5,label='downstream')
            plt.plot(xaxis,O2exp_mean,'k.',ms=5,label='average')
            plt.ylim(0,25)
            ...
            scipy.io.savemat(postfilenm,{'tauPhi':tauPhi,'M':M,'Phi':Phi,'phasor_x':phasor_x,'phasor_y':phasor_y,'O2exp':O2exp,'Rsq_ref':Rsq_ref,'Rsq_exp':Rsq_exp,'xaxis':xaxis,'yaxis':yaxis,'tauPhi_plot':tauPhi_plot,'O2exp_plot':O2exp_plot})
